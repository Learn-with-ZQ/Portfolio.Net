...
[Route("api/degree-levels")]
public sealed class DegreeLevelsController : ApiControllerBase
{
    private readonly IDegreeLevelService _service;

    public DegreeLevelsController(IDegreeLevelService service) => _service = service;
    ...
    [HttpGet("{id:int}", Name = nameof(GetDegreeLevelById))]
    [Authorize(Policy = RolePolicies.ReadAccess)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetDegreeLevelById(int id, CancellationToken cancellationToken)
    ...
    public async Task<IActionResult> Create([FromBody] CreateDegreeLevelRequest request, CancellationToken cancellationToken)
        => FromCommandResult(
            await _service.CreateAsync(request, cancellationToken).ConfigureAwait(false),
            nameof(GetDegreeLevelById));
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateDegreeLevelRequest request, CancellationToken cancellationToken)
    {
        if (id != request.DegreeLevelId)
