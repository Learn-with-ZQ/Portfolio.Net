...
public sealed class TechnologiesController : ApiControllerBase
{
    ...
    [HttpGet("{id:int}", Name = nameof(GetTechnologyById))]
    [Authorize(Policy = RolePolicies.ReadAccess)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetTechnologyById(int id, CancellationToken cancellationToken)
    ...
    public async Task<IActionResult> Create([FromBody] CreateTechnologyRequest request, CancellationToken cancellationToken)
        => FromCommandResult(
            await _service.CreateAsync(request, cancellationToken).ConfigureAwait(false),
            nameof(GetTechnologyById));
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateTechnologyRequest request, CancellationToken cancellationToken)
    {
        if (id != request.TechnologyId)
