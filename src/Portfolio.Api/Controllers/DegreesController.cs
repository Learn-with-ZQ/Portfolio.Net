...
[Route("api/degrees")]
public sealed class DegreesController : ApiControllerBase
{
    private readonly IDegreeService _service;

    public DegreesController(IDegreeService service) => _service = service;
    ...
    [HttpGet("{id:int}", Name = nameof(GetDegreeById))]
    [Authorize(Policy = RolePolicies.ReadAccess)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetDegreeById(int id, CancellationToken cancellationToken)
    ...
    public async Task<IActionResult> Create([FromBody] CreateDegreeRequest request, CancellationToken cancellationToken)
        => FromCommandResult(
            await _service.CreateAsync(request, cancellationToken).ConfigureAwait(false),
            nameof(GetDegreeById));
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateDegreeRequest request, CancellationToken cancellationToken)
    {
        if (id != request.DegreeId)
