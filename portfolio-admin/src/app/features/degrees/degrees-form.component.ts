...
@Component({
  ...
  styleUrl: './degrees-form.component.scss'
})
export class DegreesFormComponent implements OnInit {
  ...
  private loadEntity(id: number): void {
    ...
    this.api.getById(id).subscribe({
      ...
      error: (err: Error) => {
        this.snackBar.open(err.message ?? 'Failed to load degree', 'Close', { duration: 5000 });
        this.loading.set(false);
      }
    });
  }
  ...
  save(): void {
    ...
    const base = {
      degreeName: v.degreeName
    };

    const request$ = this.isEdit()
      ? this.api.update(this.entityId!, { ...base, degreeId: this.entityId!, rowVersion: this.rowVersion })
      : this.api.create(base);

    request$.subscribe({
      next: () => {
        this.snackBar.open(this.isEdit() ? 'Degree updated' : 'Degree created', 'Close', { duration: 3000 });
        void this.router.navigate(['/degrees']);
      },
      error: (err: Error) => {
