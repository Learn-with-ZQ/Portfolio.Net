...
@Component({
  ...
  styleUrl: './document-types-form.component.scss'
})
export class DocumentTypesFormComponent implements OnInit {
  ...
  private loadEntity(id: number): void {
    ...
    this.api.getById(id).subscribe({
      ...
      error: (err: Error) => {
        this.snackBar.open(err.message ?? 'Failed to load document type', 'Close', { duration: 5000 });
        this.loading.set(false);
      }
    });
  }
  ...
  save(): void {
    ...
    const request$ = this.isEdit()
      ? this.api.update(this.entityId!, { ...base, documentTypeId: this.entityId!, rowVersion: this.rowVersion })
      : this.api.create(base);

    request$.subscribe({
      next: () => {
        this.snackBar.open(this.isEdit() ? 'Document type updated' : 'Document type created', 'Close', { duration: 3000 });
        void this.router.navigate(['/document-types']);
      },
      error: (err: Error) => {
