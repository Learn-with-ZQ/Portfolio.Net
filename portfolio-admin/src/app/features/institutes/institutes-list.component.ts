...
import { Institute } from '../../core/models/institute.models';
...
export class InstitutesListComponent {
  ...
  readonly items = signal<Institute[]>([]);
  ...
  readonly displayedColumns = ['instituteName', 'location', 'actions'];
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        next: (result: PagedResult<Institute>) => {
          this.items.set(result.items);
          this.totalRecords.set(result.totalRecords);
          this.loading.set(false);
        },
        error: (err: Error) => {
          this.error.set(err.message ?? 'Failed to load institutes');
          this.loading.set(false);
        }
      });
  }
  ...
  deleteItem(item: Institute): void {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: 'Delete Institute',
        message: `Are you sure you want to delete "${item.instituteName}"?`
      }
    });
    ...
    ref.afterClosed().subscribe((confirmed) => {
      ...
      this.api.delete(item.instituteId).subscribe({
        next: () => {
          this.snackBar.open('Institute deleted', 'Close', { duration: 3000 });
          this.load();
        },
        error: (err: Error) => {
