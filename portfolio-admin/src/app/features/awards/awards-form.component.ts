...
export class AwardsFormComponent implements OnInit {
  ...
  private loadEntity(id: number): void {
    ...
    this.api.getById(id).subscribe({
      ...
      error: (err: Error) => {
        this.snackBar.open(err.message ?? 'Failed to load award', 'Close', { duration: 5000 });
        this.loading.set(false);
      }
    });
  }
  ...
  save(): void {
    ...
    const request$ = this.isEdit()
      ? this.api.update(this.entityId!, { ...base, awardId: this.entityId!, rowVersion: this.rowVersion })
      : this.api.create(base);

    request$.subscribe({
      next: () => {
        this.snackBar.open(this.isEdit() ? 'Award updated' : 'Award created', 'Close', { duration: 3000 });
