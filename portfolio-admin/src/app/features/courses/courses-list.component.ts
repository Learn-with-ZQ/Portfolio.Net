...
export class CoursesListComponent {
  ...
  readonly displayedColumns = ['courseName', 'instituteName', 'actions'];
  ...
  deleteItem(item: Course): void {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: 'Delete Course',
        message: `Are you sure you want to delete "${item.courseName}"?`
      }
    });
    ...
    ref.afterClosed().subscribe((confirmed) => {
      ...
      this.api.delete(item.courseId).subscribe({
        next: () => {
