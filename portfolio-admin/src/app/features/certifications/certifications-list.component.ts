...
export class CertificationsListComponent {
  ...
  readonly displayedColumns = ['certificationName', 'issuerName', 'issueDate', 'expiryDate', 'actions'];
  ...
  load(): void {
    ...
    this.api
      ...
      .subscribe({
        next: (result: PagedResult<Certification>) => {
          this.items.set(result.items);
          this.totalRecords.set(result.totalRecords);
          this.loading.set(false);
        },
        error: (err: Error) => {
          this.error.set(err.message ?? 'Failed to load certifications');
          this.loading.set(false);
        }
      });
  }
  ...
  deleteItem(item: Certification): void {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: 'Delete Certification',
        message: `Are you sure you want to delete "${item.certificationName}"?`
      }
    });
    ...
    ref.afterClosed().subscribe((confirmed) => {
      ...
      this.api.delete(item.certificationId).subscribe({
        next: () => {
          this.snackBar.open('Certification deleted', 'Close', { duration: 3000 });
          this.load();
        },
        error: (err: Error) => {
